...
1. `templates/tts-reader.md`를 Obsidian vault에 복사
2. vault 루트에 `obsidian-tts-config.md` 생성 후 서버 주소 설정
   (리더 노트는 이 설정 파일의 `window.ObsidianTTSConfig`를 읽으며, 설정이 없으면 동작하지 않습니다)
   ```javascript
   window.ObsidianTTSConfig = {
       operationMode: 'local',
       edgeServerUrl: 'http://<서버-IP>:5051',
       localEdgeTtsUrl: 'http://<서버-IP>:5051/api/tts',
       // ... 전체 예시는 아래 "Obsidian Vault 설정" 참조
   };
   ```
3. Obsidian에서 노트를 열고 재생 버튼 클릭

> **모듈형 설정 (권장)**: `views/` 폴더 전체를 vault에 복사하면 각 기능이 독립 모듈로 동작합니다.
> 자세한 내용은 [Obsidian 설정 가이드](#obsidian-vault-설정) 참조.
...
### 설정 시스템 (ConfigResolver)

4가지 설정 소스를 우선순위대로 병합하는 SSOT(Single Source of Truth) 모듈:
...
**operationMode** 에 따른 엔드포인트 자동 라우팅:

| Mode | TTS 요청 | 동기화 요청 |
|------|----------|------------|
| `local` | localhost:5051 | localhost:5051 |
| `server` | Azure Function | Azure Function |
| `hybrid` | localhost:5051 | SSE 활성 → localhost / 비활성 → Azure |
...
`templates/tts-reader.md`를 vault에 복사합니다. 이 리더 노트는 아래
`obsidian-tts-config.md`의 설정을 읽어 동작하므로, **설정 파일을 함께 생성**해야 합니다.

### 방법 B: 모듈형 설치 (권장)

전체 `views/` 폴더를 vault에 설치하면 기능별 독립 모듈로 동작하며 유지보수가 쉽습니다.
...
│   ├── tts-engine/view.js
│   └── ...
├── obsidian-tts-config.md     # 설정 파일 (아래 참조)
└── tts-reader.md              # TTS 리더 노트
```

### 설정 파일: obsidian-tts-config.md

vault 루트에 `obsidian-tts-config.md`를 생성합니다:
...
> `<서버-IP>` 를 실제 서버 주소로 변경하세요.
> - 로컬 전용: `localhost` 또는 `127.0.0.1`
> - Tailscale: `100.x.x.x` (Tailscale IP)
> - Cloudflare: `https://tts.yourdomain.com`

### 필수 Obsidian 플러그인

- **[Dataview](https://github.com/blacksmithgu/obsidian-dataview)**: TTS 모듈 실행 엔진
  - 설정 → Community Plugins → "Dataview" 검색 → 설치
  - 설정에서 "Enable JavaScript Queries" 활성화 필수
