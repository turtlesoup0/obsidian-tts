...
// 원리: container.innerHTML="" 후 캐시된 table DOM을 현재 container에 재부착
// 조건: dv.pages() 해시가 동일하면 (메타데이터 변경 없음) → 재부착 + 스킵
//       해시가 다르면 (노트 추가/삭제/태그 변경) → 전체 재렌더링
// ================================================================
...
const saveCacheWhenReady = () => {
    ...
    window.ttsLog?.(`📌 [통합노트] 캐시 저장 완료 (${table.querySelectorAll('tbody tr').length}행)`);
};
